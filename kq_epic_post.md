...
- Slaying the Query Dragon: The Quest for a Greener Scorecard _(i like this one, but, like, should it be more Lord of the Rings-y? 🐉)_
...
A common trait among these killed queries was a non-performant join on the `issues` table. Because pull requests continue to track their state with an issue under the hood, querying a pull request by its state required us to also query the `issues` table. This resulted in queries that looked something like this:
```
SELECT
    COUNT (*)
  FROM
